...
void Command(const char* zFormat, ...) {
    ...
    vsnprintf_s(zOutput, _TRUNCATE, zFormat, vaList);
    va_end(vaList);
    ...
    long Check(const std::string& test) {
        ...
        else if (EFFECT)
        {
            ...
            if (TYPE != AA)
            {
                ...
                const auto reuseReadyAt = pLocalPC->GetCombatAbilityTimer(EFFECT->ReuseTimerIndex, EFFECT->SpellGroup);
                if (reuseReadyAt > time(nullptr)) return 0x16; // discipline timer not ready
            }
...
PLUGIN_API unsigned long OnIncomingChat(char* Line, unsigned long Color)
{
    ...
    strncpy_s(szLine, Line ? Line : "", _TRUNCATE);
